...
bigrdd = sc.textFile(sys.argv[2])
big_rdd = bigrdd.map(lambda x : x.split(','))


threshold=int(sys.argv[1])
header=big_rdd.first()
brdd = big_rdd.filter(lambda x: x!= header).groupByKey().map(lambda x : list(set(x[1])))
total_len=brdd.count()
...
partitions=brdd.getNumPartitions()
...
candidate_itemsets=brdd.mapPartitions(lambda x : A_Priori(x,total_len)).flatMap(lambda x : x).distinct().collect()
...
frequent_itemsets=brdd.mapPartitions(lambda x : SON_phase2(x,candidate_itemsets)).reduceByKey(lambda x,y : x+y).filter(lambda x : x[1]>=threshold).map(lambda x : x[0]).collect()
...
for i in frequent_itemsets:
  if(len(i)==1):
    i=str(i)
    oneset.append(i[0:-2]+i[-1]); # Removing the comma here
    #oneset.append("("+i+")")
  else:
    result[len(i)].append(str(i))
